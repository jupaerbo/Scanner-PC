...
rem Obtener la fecha y la hora actual (compatible con todas las versiones)
...
if !IS_ADMIN! equ 0 (
    set output_file=!OUTPUT_DIR!\%machine_name%_%current_date%_%current_time%.txt
) else (
    set output_file=!OUTPUT_DIR!\%machine_name%_%current_date%_%current_time%.txt
)
...
wmic bios get serialnumber | findstr /V "SerialNumber" | findstr /V "^$" >> %output_file%
wmic bios get manufacturer,smbiosbiosversion,releasedate | findstr /V "Manufacturer" | findstr /V "^$" >> %output_file%
echo. >> %output_file%
echo ** Windows ** >> %output_file%
wmic os get caption,version,buildnumber,serialnumber | findstr /V "BuildNumber" | findstr /V "^$" >> %output_file%
if !PS_AVAILABLE! equ 1 (
